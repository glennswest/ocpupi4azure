...
az storage account create --location "East US" --name sagswx1x --kind Storage --resource-group gswx1  --sku Standard_LRS
az storage container create --name vhd --account-name sagswx1x
ACCOUNT_KEY=$(az storage account keys list --account-name sagswx1x --resource-group gswx1 --query "[0].value" -o tsv)
az storage blob copy start --account-name "sagswx1x" --account-key "$ACCOUNT_KEY" --destination-blob "$VHD_NAME" --destination-container vhd --source-uri "https://openshifttechpreview.blob.core.windows.net/rhcos/$VHD_NAME"
